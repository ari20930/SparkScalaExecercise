 ...
 * 
 */
object example5_excercise6_topassfilepath {
  def main(args:Array[String])=
  {
    val sc = new SparkContext(new SparkConf().setAppName("example5.5")
    //.setMaster("local")
        )
   ...
   val rdd = sc.textFile(fileparm)
   
   //or val rdd = sc.textFile(args(0))
   ...
   {
     println("File path parameter not passed from CLI")
   }
  }
